...
class TimerList(list):
    ...
    def append(self, item):
        ...
        if len(self)<self.maxsz:
            ...
            if n:
                ...
                raise ValueError #, print('could not append item')
    ...
    def __setitem__(self,  index, item):
        ...
        if type(item) == tuple and len(item) == 2  and type(item[0]) == float and item[0]>=time.time():
            ...
            raise TypeError #, print('invalid entry')
...
class ScanLogger(object):
    ...
    scan_types = {0: 'TCP null',
                  ...
                  TH_ACK: 'TCP ack',
                  TC_URG|TH_PSH|TH_FIN: 'TCP x-mas', TC_URG|TH_PSH|TH_FIN|TH_ACK: 'TCP x-mas',
                  TH_SYN|TH_FIN: 'TCP syn/fin',
                  TH_FIN|TH_ACK: 'TCP fin/ack',
                  TH_SYN|TH_ACK|TH_RST: 'TCP full-connect',
                  TC_URG|TH_PSH|TH_ACK|TH_RST|TH_SYN|TH_FIN: 'TCP all-flags' }

    def __init__(self, timeout, threshold, maxsize, daemon=True, logfile='/home/nics/Desktop/python_scan_logger/scanlog'):
        ...
        try:
            ...
            print("Error opening scan log file %s => %s" % (logfile))
            self.scanlog = None
    ...
    def log_scan(self, scan, continuation=False):
        ...
        if not continuation:
            line = '[%s]: %s scan (flags:%d) from %s to %s (ports:%s)' % (get_timestamp(),
                                                                          ...
                                                                          dstip,
                                                                          ports)
            #print("Sending RST Packet to the anamoly")
            #ANAMOLY_IP = srcip
            #ANAMOLY_PORT = ports
            #Message = "dpkt.tcp.TH_SYN"
            #sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            #sock.sendto(Message, (ANAMOLY_IP, ANAMOLY_PORT))
            #print("[+] Sent Packet....")
        else:
...
def main():
    ...
    o.add_option("-f", "--logfile", dest="logfile", help="File to save logs to",
                 default="/home/nics/Desktop/python_scan_logger/scanlog")
